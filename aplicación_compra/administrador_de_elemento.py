...
def items_list(self):
    # Devuelve todos los elementos que posee el propietario.
    items = [item for item in Item.item_all() if item.owner == self]
    return items

def pick_items(self, number, quantity):
    # Selecciona una cantidad específica de artículos según su número.
...
def show_items(self):
    ...
    print(tabulate(table_data, headers=["Número", "Nombre", "Precio", "Cantidad"], tablefmt="grid"))

def _stock(self):
    # Devuelve el inventario del propietario.
